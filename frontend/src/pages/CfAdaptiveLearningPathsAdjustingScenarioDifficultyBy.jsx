...
export default function CfAdaptiveLearningPathsAdjustingScenarioDifficultyBy() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-adaptive-learning-paths-adjusting-scenario-difficulty-by-performance/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-adaptive-learning-paths-adjusting-scenario-difficulty-by-performance', project: 'AIVRTrainingScenarioGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-adaptive-learning-paths-adjusting-scenario-difficulty-by-performance/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Adaptive learning paths adjusting scenario difficulty by performance</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AIVRTrainingScenarioGenerator</p>
      </div>
