...
// Scenario randomization generating training variations from domain rules
// Feature: scenario-randomization
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AIVRTrainingScenarioGenerator" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Scenario randomization generating training variations from domain rules.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'scenario-randomization', payload, result);
    res.json({ success: true, feature: 'scenario-randomization', ...result });
  } catch (err) {
    console.error('scenario-randomization analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AIVRTrainingScenarioGenerator". Return strict JSON only.',
      `Feature: Scenario randomization generating training variations from domain rules. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'scenario-randomization', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'scenario-randomization', title: 'Scenario randomization generating training variations from domain rules', project: 'AIVRTrainingScenarioGenerator' });
});
