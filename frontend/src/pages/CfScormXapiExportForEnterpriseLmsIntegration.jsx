...
export default function CfScormXapiExportForEnterpriseLmsIntegration() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-scorm-xapi-export-for-enterprise-lms-integration/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-scorm-xapi-export-for-enterprise-lms-integration', project: 'AIVRTrainingScenarioGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-scorm-xapi-export-for-enterprise-lms-integration/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">SCORM/xAPI export for enterprise LMS integration</h1>
