...
export default function GapNoNativeVrPlatformIntegrationUnityUnreal() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-native-vr-platform-integration-unity-unreal-webxr/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-native-vr-platform-integration-unity-unreal-webxr', project: 'AIVRTrainingScenarioGenerator' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-native-vr-platform-integration-unity-unreal-webxr/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No native VR platform integration (Unity, Unreal, WebXR) beyond generic layer</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_non_ai · AIVRTrainingScenarioGenerator</p>
      </div>
